-- Create new enhanced case status enum
...
-- Update cases table with new status type
ALTER TABLE cases 
  ALTER COLUMN status TYPE case_status_new USING 
    CASE 
      WHEN status = 'open' THEN 'initial_review'::case_status_new
      WHEN status = 'closed' THEN 'completed'::case_status_new
      ELSE 'initial_review'::case_status_new
    END;
...
-- Set new default
ALTER TABLE cases ALTER COLUMN status SET DEFAULT 'initial_review';
